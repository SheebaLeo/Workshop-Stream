...
public class StreamExercise {
    ...
    public void task5(){
        ...
        Set<LocalDate> finalDates = dates;
    ...
    public void task6(){
        ...
        Person[] result = people.stream().filter(p -> p.getFirstName().equals("Erik")).collect(Collectors.toList()).toArray(new Person[0]);
    ...
    public void task10(){
        ...
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("EEEE d MMMM yyyy").withLocale(Locale.ENGLISH);
        optional = people.stream().filter(p -> p.getPersonId() == 5914).map(p-> p.getDateOfBirth())
                .map(date -> date.format(formatter).toUpperCase(Locale.ROOT))
                        .findFirst();
    ...
    public void task12(){
        ...
        result = people.stream().map(person -> person.getFirstName()).filter(isPalindrome)
                .distinct()
                .sorted()
                .toArray(String[]::new);
